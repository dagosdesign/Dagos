    ...
    <Card className="p-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <BarChart3 className="w-5 h-5" color={C.gold} strokeWidth={2.4} />
      ...
      <div className="mt-4 flex items-center gap-4 sm:gap-6">
        ...
        <div className="min-w-0 flex-1">
          {assessed ? (
            <>
              <p className="text-[40px] font-bold leading-none" style={{ color: C.gold }}>
                {profile.level}
              </p>
            ...
            <>
              ...
              <button
                ...
                className="mt-2 text-[12px] font-semibold rounded-full px-3 py-1.5 cursor-pointer"
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-[25px] font-bold leading-none" style={{ color: C.text }}>
          {label}
        </span>
        <span className="text-[12px] mt-1" style={{ color: C.muted }}>
          Progress
        </span>
      </div>
    </div>
  ...
  return (
    <div className="mt-3">
      <div className="relative flex items-center justify-between">
        ...
        {CEFR_LEVELS.map((l, i) => (
          <span
            ...
            style={
              i === current
                ? { width: 12, height: 12, background: C.gold, boxShadow: '0 0 10px rgba(245,184,46,0.6)' }
                : { width: 10, height: 10, background: '#3A3A3A' }
            }
          />
        ))}
      </div>
      <div className="flex justify-between mt-2">
        {CEFR_LEVELS.map((l, i) => (
          <span key={l} className="text-[11.5px] w-6 text-center" style={{ color: i === current ? C.gold : C.muted }}>
            {l}
          </span>
        ))}
      </div>
